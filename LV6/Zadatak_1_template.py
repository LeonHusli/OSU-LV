...
y_train_p_KNN1 = KNN1_model.predict(X_train_n)
y_test_p_KNN1 = KNN1_model.predict(X_test_n)
...
y_train_p_KNN100 = KNN100_model.predict(X_train_n)
y_test_p_KNN100 = KNN100_model.predict(X_test_n)
...
for K in K_values:
    KNNx_model = KNeighborsClassifier(n_neighbors=K)
    scores = cross_val_score(KNNx_model, X_train_n, y_train, cv=5)
    cv_scores.append(scores.mean())

best_K = K_values[cv_scores.index(max(cv_scores))]

print("Najbolji K:", best_K)
print("Najbolja CV točnost:", max(cv_scores))
...
grid = GridSearchCV(SVMx_model, param_grid, cv=5)
grid.fit(X_train_n, y_train)
pred = grid.predict(X_test_n)

print("Najbolji parametri:", grid.best_params_)
print("Najbolja točnost:", grid.best_score_)
print("Tocnost: " + "{:0.3f}".format((accuracy_score(y_test, pred))))